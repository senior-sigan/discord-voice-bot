...
### Поле чудес — Discord Activity

Вместе с ботом на `127.0.0.1:7071` запускается общая iframe-игра в духе DOS-версии 1993 года: одно слово из встроенного пула, 16 секторов, четвертьфинал, полуфинал, финал и нелепый главный приз. Буквы, вращение и ответ принимаются кнопками или голосом без wake word: «буква А», «крути барабан», «ответ — капибара». Для локальной проверки откройте `http://127.0.0.1:7071`. Для запуска внутри Discord:

1. В Developer Portal включите **Activities** у приложения Олега.
2. Запустите туннель: `ngrok http 7071`.
3. Добавьте URL Mapping `/` на выданный ngrok HTTPS-домен.
4. Запустите Олега с подключением к голосовому каналу: `npm start -- --autojoin`.
5. Откройте Activity через созданную Discord команду Entry Point.

Игровое состояние хранится в памяти процесса и разделяется по Discord `instance_id`; после перезапуска раунд начнётся заново. При входе участник выбирает свой Discord-профиль — его `user_id` связывает голосовую расшифровку с игроком на экране. Сервер проверяет публичные instance ID через Discord API, поэтому обычная ссылка ngrok не позволяет управлять игрой. Для прямой локальной страницы используется отдельная комната `local`; голосовая привязка в ней не используется.
